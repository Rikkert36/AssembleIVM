...
namespace AssembleIVM.GJTComputerFiles.ConstructorStructures.nodes {
    class GJTQuickMinusNode : GJTInnerNode {
        public GJTQuickMinusNode(string name, List<string> variables, List<GJTNode> children, List<TreeNode> predicates, Enumerator enumerator, string orderDimension = "") : base(name, variables, children, predicates, enumerator, orderDimension) {
        }
        ...
        public override NodeReduct GenerateReduct(string modelName) {
            ...
            QuickMinusNode result = new QuickMinusNode(this.name, this.variables, reductChildren, this.predicates, this.enumerator, this.inFrontier);
